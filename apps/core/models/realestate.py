...
class ObjectType(models.TextChoices):
    LAND_PLOT = "Земельна ділянка", "Земельна ділянка"
    RESIDENTIAL_BUILDING = "Житловий будинок", "Житловий будинок"
    HOUSEHOLD = "Домоволодіння", "Домоволодіння"
    APARTMENT = "Квартира", "Квартира"
    ROOM = "Кімната", "Кімната"
    OFFICE = "Офіс", "Офіс"
    GARDEN_HOUSE = "Садовий (дачний) будинок", "Садовий (дачний) будинок"
    GARAGE = "Гараж", "Гараж"
    PARKING_SPACE = "Машиномісце (паркомісце)", "Машиномісце (паркомісце)"
    NON_RESIDENTIAL = "Нежитлове приміщення", "Нежитлове приміщення"
    OTHER = "Інше", "Інше"
...
class RealEstateDeclaration(models.Model):
    ...
    iteration = models.CharField(max_length=100)  # id нерухомості з API НАЗК
...
class RealEstate(models.Model):
    # Звичайні поля
    object_type = models.CharField(max_length=50, choices=ObjectType.choices, null=True, blank=True) # вид обєкта
    actual_country = models.PositiveSmallIntegerField(choices=CountryRealEstate.choices, null=True, blank=True) # країна
    region = models.CharField(max_length=255, null=True, blank=True) # область
    city = models.CharField(max_length=255, null=True, blank=True) # населений пункт
    total_area = models.DecimalField(max_digits=100, decimal_places=2, null=True, blank=True) # загальна площа
    cost_date_assessment = models.CharField(max_length=100, null=True, blank=True) # вартість на дату набуття
    owning_date = models.CharField(max_length=100, null=True, blank=True) # дата набуття права
    ...
    def get_owners_display(self):
        display_list = []
        for owner in self.owners.all():
            display_list.append(str(owner))
        return "; ".join(display_list)

    def __str__(self):
        return f"{self.object_type} у {self.city}, площа {self.total_area} м². Власники: {self.get_owners_display()}"